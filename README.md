...
ранжирование NetVLAD и локальное сопоставление SuperPoint/LightGlue и умеет
обогащать ответы адресами через Nominatim. Сервис хранит исходные изображения и
все бинарные артефакты в S3, а нормализованные дескрипторы и метаданные — в
PostgreSQL. Благодаря чистой архитектуре систему легко расширять под новые
...
- **S3 + PostgreSQL из коробки.** В хранилище лежат исходники, глобальные и
  локальные дескрипторы (`npz`), в базе — нормализованные векторы, ключи и
  произвольные JSON-метаданные.
- **Проработанное API.** REST-эндпоинты позволяют загружать снимки, искать
  совпадения, получать визуализации keypoint-ов и результаты обратного
  геокодирования.
...
1. **Ингест**: изображение поступает через API или CLI, вычисляются локальные и
   глобальные признаки, объекты сериализуются в S3, а запись с нормализованным
   вектором попадает в PostgreSQL.
2. **Поиск**: NetVLAD вычисляет глобальный дескриптор запроса, косинусная
   близость выбирает кандидатов, LightGlue подтверждает совпадения и формирует
   итоговый скоринг.
...
> Для GPU-окружений установите PyTorch с нужной сборкой CUDA и включите
> `PREFER_GPU=true`.
...
| `S3_*` | Параметры S3-совместимого хранилища и префиксы объектов. |
...
- `POST /v1/search_by_image` — найти топ-N совпадений; параметр `plot_dots=true`
  включает отрисовку ключевых точек SuperPoint на результатах.
  ...
  "matches": [
    {
      ...
      "geometry_score": 0.5,
      "image_uri": "s3://geo-images/images/…",
      "feature_uri": "s3://geo-images/features/local/…",
      "latitude": 55.75,
      "longitude": 37.61,
